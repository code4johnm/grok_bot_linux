...
**Grok Bot companion** — a launcher TUI only. It does **not** replace the Electron Grok Bot desktop.

Status line: **grok-bot-tui · Grok Bot companion**.

This repo packages **two** products: (A) the community Electron desktop (`launch.sh` / `grok-bot` / `chrome-sandbox` from [https://x.ai/bot](https://x.ai/bot)) and (B) the official `grok` CLI (Grok Build TUI). A Python replacement of the Electron binary was already tried (PR #2) and reverted. This package does not replace, rename, or remove that tree. Installers and `app/` stay with the desktop packager.
...
| **Grok Bot** | Teammate product ([https://x.ai/bot](https://x.ai/bot), [launch post](https://x.ai/news/introducing-grok-bot)): persistent bots, own computer, approvals, routines | **Launch this.** |

On Linux this repo already packages (1) the community Grok Bot desktop as `grok-bot` and (2) the official CLI `grok` (help text starts **Grok Build TUI**; often `~/.grok/bin/grok`). This process does not invent a second chat product.

## What this is

1. `/gui` starts the **packaged Electron desktop**: `grok-bot` on PATH or this tree's `launch.sh` (those apply `chrome-sandbox` / `--no-sandbox`), else [https://x.ai/bot](https://x.ai/bot). Never grok.com.
...
- Not a replacement for Electron `grok-bot`. Not Grok the chat app, not a rebrand of grok.com.
...
| `/gui` | Launch packaged `grok-bot` or `launch.sh`, else [https://x.ai/bot](https://x.ai/bot) |
| `/help` | List commands |
...
```text
Grok Bot companion  grok-bot-tui 0.1.5
Grok Bot companion. /gui → grok-bot/launch.sh or https://x.ai/bot. Default: official grok CLI.
grok> /gui
Launched Grok Bot desktop: /usr/local/bin/grok-bot
grok> /grok --help
